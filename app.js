(function () {
    ...
    if (!coords) {
      ...
      if (at >= 0) {
        ...
        if (parts.length >= 2) {
          ...
          if (la !== null && ln !== null && Math.abs(la) <= 90 && Math.abs(ln) <= 180) coords = { lat: la, lng: ln };
        }
      }
    }
    if (!coords) {
  ...
  function setupUI() {
    ...
    go.onclick = function () {
      ...
      if (!isNaN(la) && !isNaN(ln)) initPano(la, ln, { clickToGo: chkClick.checked, linksControl: chkLinks.checked });
    };
  ...
  async function startRecording(btnStart) {
    ...
    if (!stream && navigator.mediaDevices && navigator.mediaDevices.getDisplayMedia) {
      alert("Konnte Canvas-Stream nicht direkt abgreifen. Bitte wähle im folgenden Dialog den Tab/Fensterbereich aus.");
      try {
    ...
    REC.rec.onstop = function () {
      ...
      console.log("Aufnahme gestoppt! Konvertiere Video...");
      transcodeToMp4(blob);
      safeStopStream();
    };
    REC.rec.start(100);
